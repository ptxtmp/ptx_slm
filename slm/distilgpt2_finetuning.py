...
# Add proper multiprocessing support for Windows
if __name__ == '__main__':
    ...
    # Streamlined dataset preparation - removed multiprocessing
    def load_and_prepare_dataset(file_path, tokenizer):
        dataset = load_dataset('text', data_files=file_path)  # Removed num_proc
        return dataset['train'].map(
            ...
            batched=True,
            # Removed num_proc to avoid multiprocessing issues
            remove_columns=['text']
        )
    ...
    test_prompt = "What is the goal of textual criticism"
